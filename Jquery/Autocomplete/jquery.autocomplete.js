...
;(function($) {
	...
	$.Autocompleter.defaults = {
		...
		highlight: function(value, term) {
			// ACCENT FIX
			var p,l=term.length,res='',_value=remove_accent(value).toLowerCase(),_term=remove_accent(term).toLowerCase();while((p=_value.indexOf(_term))!=-1){res+=value.substr(0,p)+'<strong>'+value.substr(p,l)+'</strong>';value=value.substr(p+l);_value=_value.substr(p+l);}return res+value;
		},
	    scroll: true,
	    scrollHeight: 180
	};
